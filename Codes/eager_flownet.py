...
input_a = np.array([im1]).astype(np.float32)
input_b = np.array([im2]).astype(np.float32)

with tfe.restore_variables_on_create(const.FLOWNET_CHECKPOINT):
    # input size is 384 x 512
    input_a = tf.image.resize_images(input_a, [flow_height, flow_width])
    input_b = tf.image.resize_images(input_b, [flow_height, flow_width])
    flows = net.model(
        inputs={'input_a': input_a, 'input_b': input_b},
        training_schedule=LONG_SCHEDULE,
        trainable=False, reuse=None
    )['flow']
with tfe.restore_variables_on_create(const.FLOWNET_CHECKPOINT):
    flows2 = net.model(
        inputs={'input_a': input_a, 'input_b': input_b},
...
flow_im = flow_to_image(flows[0, ...].numpy())
flow_im2 = flow_to_image(flows2[0, ...].numpy())
plt.imshow(flow_im)
plt.show()
plt.imshow(flow_im2)
plt.show()
